...
| Campo | Valor |
|---|---|
| SIMULATION-ID | SIM-TASK-006-USER-C-E2 |
| USER | USER-C - Cazador experto |
| TASK | TASK-006 - Ir a la tienda (outbound) |
| ENTRY | `/` (salvo gates) |
...
- Home ATTENTION-1: price
- Home ATTENTION-2: store
- Home ATTENTION-3 (posible ignore): decorative_icons
- Detail ATTENTION-1/2/3: precio/descuento / tienda/votos / affiliate disclosure / condiciones (posible ignore)
...
- USER INTERPRETATION: Cazador experto: interpreta Home como punto de partida para ofertas.
- ACTION: Orientarse
...
### Step 2 - DISCOVERY
- SCREEN: /
- VISIBLE INFORMATION: Cards OfferCard; tabs; ATTENTION-1=price; ATTENTION-2=store; ATTENTION-3(posible ignore)=decorative_icons
- USER INTERPRETATION: Cree que puede scrollear/filtrar/abrir cards. Prioriza: price, store, votes, expiry_freshness. Puede ignorar: decorative_icons.
- ACTION: Scroll + inspeccionar cards
- INTERACTION TYPE: INT-SCROLL
- DECISION: Usar modo top + timeFilter
- REASON: Perfil USER-C
- UNCERTAINTY: MEDIUM
...
### Step 4 - ACTION
- SCREEN: /oferta/[id]
- VISIBLE INFORMATION: CTA Ver si sigue disponible / outbound
- USER INTERPRETATION: Cree que abre la tienda externa.
- ACTION: Activar CTA outbound
- INTERACTION TYPE: INT-OUTBOUND
- DECISION: Ir a tienda
- REASON: Tras evaluacion positiva/parcial
- UNCERTAINTY: LOW
- SYSTEM RESPONSE: trackAndOpenOfferUrl → track-outbound → window.open (degrada sin clickId si track falla)
- NEXT STEP: external
- ERROR: SIM-ERR-0
- RECOVERY: n/a
...
- INT-004: TYPE=OUTBOUND; TARGET=Detail CTA; PURPOSE=ACTION; RESULT=window.open

## Decisions log
- DEC-D1: OPTIONS=[vitales, top, latest, personalized?, categoria]; CHOSEN=top + timeFilter; REASON=Sesgo USER-C; CONFIDENCE=MEDIUM
- DEC-06: OPTIONS=[ignorar, abrir, votar en card, fav en card]; CHOSEN=abrir; REASON=Tienda conocida + votos; CONFIDENCE=MEDIUM
- DEC-10: OPTIONS=[salir, outbound, favorito]; CHOSEN=outbound; REASON=Objetivo tarea; CONFIDENCE=HIGH
