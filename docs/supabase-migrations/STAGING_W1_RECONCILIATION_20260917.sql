-- =============================================================================
-- STAGING W1 RECONCILIATION — 20260917
-- TARGET PROJECT-REF (ONLY): oojshofrpbfwsiypcecr
-- FORBIDDEN: mkgsrpsuvedwwlzmzmzh (production)
--
-- Derived from: FOUNDATION_BASELINE_20260917.sql + Gate 0.5 forensics
-- Docs: STAGING_W0_BACKUP_PLAN.md, STAGING_PROFILES_RECONCILIATION.md
--
-- NO DROP TABLE / TRUNCATE / destructive CASCADE
-- NO money / Supply / attribution / distribution
-- NO communities uuid conversion (deferred)
-- ofertas data (5 rows) NOT migrated into offers (gap documented)
-- =============================================================================

BEGIN;

-- ---------------------------------------------------------------------------
-- 0) Hard abort unless we can see staging-only markers in-session
--     (caller MUST use project_id=oojshofrpbfwsiypcecr)
-- ---------------------------------------------------------------------------
DO $$
BEGIN
  -- Soft heuristic: staging still has legacy ofertas + offers as view before rename
  IF to_regclass('public.ofertas') IS NULL THEN
    RAISE EXCEPTION 'W1 ABORT: expected legacy public.ofertas on staging';
  END IF;
END $$;

-- ---------------------------------------------------------------------------
-- 1a) Rename VIEW offers → offers_legacy_compat_v (preserve definition)
-- ---------------------------------------------------------------------------
DO $$
DECLARE k "char";
BEGIN
  ...
  IF k = 'v' THEN
    ALTER VIEW public.offers RENAME TO offers_legacy_compat_v;
  ELSIF k = 'r' THEN
    RAISE NOTICE 'W1: public.offers already a TABLE — skip rename';
  ELSIF k IS NULL THEN
    RAISE NOTICE 'W1: public.offers absent — will CREATE TABLE';
  ELSE
    RAISE EXCEPTION 'W1 ABORT: public.offers unexpected relkind=%', k;
  END IF;
END $$;

-- ---------------------------------------------------------------------------
-- 1b) Rename VIEW user_roles → user_roles_from_profiles_v
-- ---------------------------------------------------------------------------
DO $$
DECLARE k "char";
BEGIN
  ...
  IF k = 'v' THEN
    ALTER VIEW public.user_roles RENAME TO user_roles_from_profiles_v;
  ELSIF k = 'r' THEN
    RAISE NOTICE 'W1: public.user_roles already a TABLE — skip rename';
  ELSIF k IS NULL THEN
    RAISE NOTICE 'W1: public.user_roles absent — will CREATE TABLE';
  ELSE
    RAISE EXCEPTION 'W1 ABORT: public.user_roles unexpected relkind=%', k;
  END IF;
END $$;

-- Recreate effective view against renamed legacy projection + tbl
-- (will be replaced again after canonical user_roles table exists)
CREATE OR REPLACE VIEW public.user_roles_effective AS
  SELECT user_id, role FROM public.user_roles_from_profiles_v
  UNION
  SELECT user_id, role FROM public.user_roles_tbl;

-- ---------------------------------------------------------------------------
-- 2) profiles — additive canonical columns + deterministic mappings
-- ---------------------------------------------------------------------------
ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS username text;
ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS display_name text;
ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS onboarding_completed boolean NOT NULL DEFAULT false;
ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS offers_submitted_count integer NOT NULL DEFAULT 0;
ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS offers_approved_count integer NOT NULL DEFAULT 0;
ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS offers_rejected_count integer NOT NULL DEFAULT 0;
ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS display_name_updated_at timestamptz DEFAULT now();
ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS reputation_score integer NOT NULL DEFAULT 0;
ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS reputation_level integer NOT NULL DEFAULT 1;
...
ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS leader_badge text;
ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS ml_tracking_tag text;
ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS preferred_categories text[] DEFAULT '{}'::text[];
ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS vote_weight_multiplier integer NOT NULL DEFAULT 1;
ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS commissions_accepted_at timestamptz;
ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS commissions_terms_version text;
ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS name_saved_in_settings_at timestamptz;
ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS owner_auto_approve_offers boolean NOT NULL DEFAULT false;
ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS owner_auto_approve_offers_at timestamptz;
ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS owner_auto_approve_offers_by uuid;
ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS commission_legal_name text;
ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS commission_rfc text;
ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS commission_clabe text;
ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS commission_fiscal_updated_at timestamptz;
ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS amazon_tracking_tag text;
ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS reward_program_unlocked_at timestamptz;
ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS welcome_offer_id uuid;
ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS welcome_offer_selected_at timestamptz;
ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS terms_accepted_at timestamptz;
ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS privacy_accepted_at timestamptz;
ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS legal_consent_version text;
ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS account_deletion_requested_at timestamptz;
ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS rewards_terms_accepted_at timestamptz;
ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS rewards_terms_version text;

-- Deterministic data mappings (see STAGING_PROFILES_RECONCILIATION.md)
UPDATE public.profiles
SET display_name = full_name
WHERE display_name IS NULL AND full_name IS NOT NULL;

UPDATE public.profiles
SET is_trusted = COALESCE(trusted, false)
WHERE true;

UPDATE public.profiles
SET offers_approved_count = COALESCE(approved_offers_count, offers_approved_count, 0)
WHERE true;
...
DO $$ BEGIN
  ALTER TABLE public.profiles
    ADD CONSTRAINT profiles_leader_badge_check
...
DO $$ BEGIN
  ALTER TABLE public.profiles
    ADD CONSTRAINT profiles_vote_weight_multiplier_check
    CHECK (((vote_weight_multiplier >= 1) AND (vote_weight_multiplier <= 1000)));
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;

-- Modern policies (coexist with legacy; DROP IF EXISTS then CREATE)
...
-- Deterministic seed: profiles.role in staff set → user_roles
INSERT INTO public.user_roles (user_id, role)
SELECT p.id, p.role
FROM public.profiles p
WHERE p.role = ANY (ARRAY['owner'::text, 'admin'::text, 'moderator'::text, 'analyst'::text])
  AND NOT EXISTS (
...
-- ---------------------------------------------------------------------------
-- 4) Canonical offers TABLE (EMPTY — no unsafe bigint→uuid data migration)
-- ---------------------------------------------------------------------------
...
DO $$ BEGIN
  ALTER TABLE public.offers
    ADD CONSTRAINT offers_locked_by_fkey
    FOREIGN KEY (locked_by) REFERENCES auth.users(id) ON DELETE SET NULL;
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;

-- welcome_offer FK (circular) — add after offers exists
DO $$ BEGIN
  ALTER TABLE public.profiles
...
DO $$ BEGIN
  ALTER TABLE public.profiles
    ADD CONSTRAINT profiles_owner_auto_approve_offers_by_fkey
    FOREIGN KEY (owner_auto_approve_offers_by) REFERENCES auth.users(id) ON DELETE SET NULL;
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;
...
COMMENT ON TABLE public.offers IS
  'W1 canonical offers (uuid). Legacy demo rows remain in public.ofertas; view offers_legacy_compat_v. No automatic data copy.';

-- ---------------------------------------------------------------------------
-- 5) Child / foundation tables (missing on staging)
-- ---------------------------------------------------------------------------
...
DO $$ BEGIN
  ALTER TABLE public.offer_votes ADD CONSTRAINT offer_votes_offer_id_fkey
    FOREIGN KEY (offer_id) REFERENCES public.offers(id) ON DELETE CASCADE;
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN
  ALTER TABLE public.offer_votes ADD CONSTRAINT offer_votes_user_id_fkey
    FOREIGN KEY (user_id) REFERENCES auth.users(id) ON DELETE CASCADE;
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
...
DO $$ BEGIN
  ALTER TABLE public.offer_events ADD CONSTRAINT offer_events_offer_id_fkey
    FOREIGN KEY (offer_id) REFERENCES public.offers(id) ON DELETE CASCADE;
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN
  ALTER TABLE public.offer_events ADD CONSTRAINT offer_events_user_id_fkey
    FOREIGN KEY (user_id) REFERENCES public.profiles(id) ON DELETE SET NULL;
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
CREATE INDEX IF NOT EXISTS idx_offer_events_offer_id ON public.offer_events (offer_id);
ALTER TABLE public.offer_events ENABLE ROW LEVEL SECURITY;
-- 0 client policies on prod (service_role path)
...
DO $$ BEGIN
  ALTER TABLE public.offer_favorites ADD CONSTRAINT offer_favorites_offer_id_fkey
    FOREIGN KEY (offer_id) REFERENCES public.offers(id) ON DELETE CASCADE;
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN
  ALTER TABLE public.offer_favorites ADD CONSTRAINT offer_favorites_user_id_fkey
...
DO $$ BEGIN
  ALTER TABLE public.comments ADD CONSTRAINT comments_offer_id_fkey
    FOREIGN KEY (offer_id) REFERENCES public.offers(id) ON DELETE CASCADE;
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN
...
DO $$ BEGIN
  ALTER TABLE public.offer_reports ADD CONSTRAINT offer_reports_offer_id_fkey
    FOREIGN KEY (offer_id) REFERENCES public.offers(id) ON DELETE CASCADE;
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN
  ALTER TABLE public.offer_reports ADD CONSTRAINT offer_reports_reporter_id_fkey
...
DO $$ BEGIN
  ALTER TABLE public.moderation_logs ADD CONSTRAINT moderation_logs_offer_id_fkey
    FOREIGN KEY (offer_id) REFERENCES public.offers(id) ON DELETE SET NULL;
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN
  ALTER TABLE public.moderation_logs ADD CONSTRAINT moderation_logs_user_id_fkey
...
DO $$ BEGIN
  ALTER TABLE public.moderation_outcomes ADD CONSTRAINT moderation_outcomes_offer_id_fkey
    FOREIGN KEY (offer_id) REFERENCES public.offers(id) ON DELETE CASCADE;
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
...
DO $$ BEGIN
  ALTER TABLE public.user_bans ADD CONSTRAINT user_bans_user_id_fkey
    FOREIGN KEY (user_id) REFERENCES auth.users(id) ON DELETE CASCADE;
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN
  ALTER TABLE public.user_bans ADD CONSTRAINT user_bans_banned_by_fkey
...
DO $$ BEGIN
  ALTER TABLE public.user_email_preferences ADD CONSTRAINT user_email_preferences_user_id_fkey
    FOREIGN KEY (user_id) REFERENCES auth.users(id) ON DELETE CASCADE;
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
...
DO $$ BEGIN
  ALTER TABLE public.plaza_requests ADD CONSTRAINT plaza_requests_user_id_fkey
    FOREIGN KEY (user_id) REFERENCES auth.users(id) ON DELETE CASCADE;
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
...
DO $$ BEGIN
  ALTER TABLE public.plaza_discussions ADD CONSTRAINT plaza_discussions_user_id_fkey
    FOREIGN KEY (user_id) REFERENCES auth.users(id) ON DELETE CASCADE;
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
...
CREATE OR REPLACE VIEW public.ofertas_ranked_general AS
 SELECT id,
    ...
    (COALESCE(upvotes_count, 0) * 2 - COALESCE(downvotes_count, 0))::double precision
      / power(GREATEST(COALESCE(EXTRACT(epoch FROM now() - created_at), 0::numeric) / 3600::numeric + 2::numeric, 2::numeric), 1.5)::double precision AS score_final,
...
-- =============================================================================
-- DATA NOT MIGRATED
-- - public.ofertas (5 demo rows) → public.offers (empty). Gap: bigint PK + incomplete cols.
-- - votos → offer_votes (0 rows; different value domain anyway)
-- - moderation_log → moderation_logs (0 rows; different shape)
-- - communities bigint (deferred)
-- =============================================================================
