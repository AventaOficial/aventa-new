...
function toRow(record: HunterCandidateRecord): Record<string, unknown> {
  return {
    ...
    experiment_variant: record.experimentVariant ?? null,
    discount_class: record.discountClass ?? null,
    discount_class_v1: record.discountClassV1 ?? null,
    discount_confidence: record.discountConfidence ?? null,
    discount_source: record.discountSource ?? null,
    historical_price_confidence: record.historicalPriceConfidence ?? null,
    price_evidence: record.priceEvidence ?? null,
    current_decision: record.currentDecision ?? record.decision ?? null,
    hypothetical_decision: record.hypotheticalDecision ?? null,
    funnel_stage: record.funnelStage ?? null,
    funnel_reason: record.funnelReason ?? null,
    would_topk_cut: record.wouldTopkCut === true,
    would_diversity_cut: record.wouldDiversityCut === true,
    persisted_pre_gate: record.persistedPreGate === true,
    discovery_count_in_run: record.discoveryCountInRun ?? 1,
    first_price_sale: record.firstPriceSale ?? null,
    last_price_sale: record.lastPriceSale ?? null,
    price_change_in_run: record.priceChangeInRun ?? null,
    rot_page: record.rotPage ?? null,
    rot_seed_id: record.rotSeedId ?? null,
    rot_category_id: record.rotCategoryId ?? null,
    rot_query: record.rotQuery ?? null,
    rot_brand: record.rotBrand ?? null,
    rot_price_band: record.rotPriceBand ?? null,
    axis_bitmap: record.axisBitmap ?? {},
