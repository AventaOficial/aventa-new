...
export type ElektraExtractResult = {
  title: string | null;
  image: string | null;
  images: string[];
  store: 'Elektra';
...
function collectElektraCdnImages(html: string, base: string): string[] {
  const out: string[] = [];
  const re =
    /(https?:\/\/(?:cdn|static|img|media)[^"'\\\s]*elektra[^"'\\\s]*\.(?:jpg|jpeg|webp|png))/gi;
...
export function extractElektraProduct(html: string, pageUrl: string): ElektraExtractResult {
  ...
  for (const u of collectElektraCdnImages(html, pageUrl)) pushUnique(images, u);
