...
/** Build/prerender may lack service-role secrets (preview projects). Never throw. */
function canQuerySitemapDb(): boolean {
...
export function getSitemapStatic(): MetadataRoute.Sitemap {
  return [
    ...
    { url: `${BASE_URL}/plaza`, lastModified: new Date(), changeFrequency: 'daily', priority: 0.7 },
...
export async function getSitemapStores(): Promise<MetadataRoute.Sitemap> {
  ...
  try {
    ...
    const storeNames = [
      ...new Set((storeRows ?? []).map((r) => r?.store).filter((s): s is string => Boolean(s?.trim()))),
    ];
