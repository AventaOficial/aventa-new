...
export async function POST(request: Request) {
  ...
  try {
    ...
    const { error } = await supabase.rpc('increment_offers_approved_count', { uuid: userId })
    if (error) {
      console.error('[reputation] increment_offers_approved_count failed:', error.message)
