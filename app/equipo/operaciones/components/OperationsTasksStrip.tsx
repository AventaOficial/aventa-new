...
export default function OperationsTasksStrip({
  board,
  taskPct,
  onTasksChange,
}: {
  ...
  const patch = async (payload: Record<string, unknown>) => {
    ...
    try {
      ...
      await fetch('/api/staff/tasks', {
        method: 'PATCH',
        headers,
        body: JSON.stringify({ department: 'operaciones', ...payload }),
      });
      onTasksChange();
    } finally {
      setSaving(false);
  ...
  return (
    <section className="rounded-2xl border border-sky-500/15 bg-sky-50/40 dark:bg-sky-950/15 p-4">
      <div className="flex items-center justify-between gap-3 mb-3">
        <h2 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Checklist del día</h2>
        <span className="text-xs font-medium text-sky-700 dark:text-sky-400 tabular-nums">{taskPct}%</span>
      </div>
      <div className="h-1.5 rounded-full bg-sky-200/60 dark:bg-sky-900/40 overflow-hidden mb-3">
        <div className="h-full rounded-full bg-sky-500 transition-all duration-500" style={{ width: `${taskPct}%` }} />
      </div>
      <ul className="space-y-2">
        {board.tasks.map((t) => (
          <li key={t.id}>
            ...
            >
              {t.done ? (
                <CheckCircle2 className="h-4 w-4 shrink-0 text-sky-500 mt-0.5" />
              ) : (
      ...
      <div className="mt-3 flex gap-2">
        ...
        <button
          ...
          className="rounded-xl bg-sky-600 text-white p-2 disabled:opacity-50"
          aria-label="Añadir"
        >
