...
export default function FinanceTasksStrip({
  board,
  taskPct,
  onTasksChange,
}: {
  ...
  const patch = async (payload: Record<string, unknown>) => {
    ...
    try {
      ...
      await fetch('/api/staff/tasks', {
        method: 'PATCH',
        headers,
        body: JSON.stringify({ department: 'contabilidad', ...payload }),
      });
      onTasksChange();
    } finally {
      setSaving(false);
  ...
  return (
    <section className="rounded-2xl border border-amber-500/15 bg-amber-50/40 dark:bg-amber-950/15 p-4">
      <div className="flex items-center justify-between gap-3 mb-3">
        <h2 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Checklist del día</h2>
        <span className="text-xs font-medium text-amber-700 dark:text-amber-400 tabular-nums">{taskPct}%</span>
      </div>
      <div className="h-1.5 rounded-full bg-amber-200/60 dark:bg-amber-900/40 overflow-hidden mb-3">
        <div className="h-full rounded-full bg-amber-500 transition-all duration-500" style={{ width: `${taskPct}%` }} />
      </div>
      ...
      <div className="mt-3 flex gap-2">
        ...
        <button
          ...
          className="rounded-xl bg-amber-600 text-white p-2 disabled:opacity-50"
          aria-label="Añadir"
        >
