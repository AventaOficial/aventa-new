...
export default function ModerationOffersHistoryPanel({
  status,
  title,
  emptyMessage,
}: Props) {
    ...
    <div>
      ...
      {loading ? (
        ...
        <div className="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-[#1a1a1a] p-8 text-center text-gray-500 dark:text-gray-400">
          {offers.length === 0 ? emptyMessage : 'No hay resultados para tu búsqueda.'}
        </div>
      ) : (
          ...
          {filtered.map((offer) => (
            <li key={offer.id}>
              <ModerationOfferCard
                offer={offer}
                status={status}
                onOfferUpdated={status === 'approved' ? () => refreshList(true) : undefined}
              />
            </li>
          ))}
        </ul>
