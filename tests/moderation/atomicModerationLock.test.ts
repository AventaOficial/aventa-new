...
describe('assertModeratorOwnsLock', () => {
  it('permite actuar sin lock previo', () => {
    expect(assertModeratorOwnsLock({ locked_by: null, locked_at: null }, 'mod-a').ok).toBe(true);
  });
  ...
  it('permite recuperar lock stale', () => {
    const stale = new Date(Date.now() - MODERATION_LOCK_STALE_MS - 1000).toISOString();
    expect(assertModeratorOwnsLock({ locked_by: 'mod-b', locked_at: stale }, 'mod-a').ok).toBe(
      true
    );
  });
});
